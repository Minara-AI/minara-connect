...
0. **Claude PID Binding gate.** Walk the parent process chain (Linux: `/proc/<pid>/stat` for ppid + `/proc/<pid>/exe` for binary; macOS: `ps -p <pid> -o ppid=,comm=`) until reaching a process whose executable basename is `claude`. If no `claude` ancestor is found within a depth bound (reference implementation: 16 levels), the Hook **MUST** emit nothing on stdout and exit `0` — this is the canonical "no claude parent → no injection" path for any unrelated invocation on the same machine. Once a Claude PID `P_C` is identified, read `~/.cc-connect/sessions/by-claude-pid/<P_C>/rooms.json` (schema: `{"v":1,"topics":[<topic_hex>, ...]}`, file mode `0600`, parent dirs `0700`). If the file is missing or its `topics` list is empty, emit nothing and exit `0` — that Claude is running but not participating in any Room. Implementations **MUST** opportunistically prune `~/.cc-connect/sessions/by-claude-pid/<pid>/` directories whose owning PID is no longer alive (per `kill(pid, 0) == ESRCH`) on every Hook fire; this caps the unbounded growth of orphaned state when Claude exits without an explicit `cc_leave_room`. Pre-v0.6 implementations gated on the `CC_CONNECT_ROOM` env var; that variable is **DEPRECATED in v0.6** and **MUST** be ignored by Hook implementations conforming to this version of the spec. After step 1 enumerates `*.active` files, implementations **MUST** filter the list to topics that appear in `rooms.json`'s `topics` array before proceeding to step 3 — a topic in `rooms.json` whose chat-daemon has crashed produces no useful injection and is silently skipped.
...
2. List `*.active` files in that directory. For each:
   ...
   - **MUST** drop entries whose topic_id_hex does not appear in the rooms.json `topics` array (step 0).
