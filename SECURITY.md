...
| Cross-process Claude isolation | The `UserPromptSubmit` hook only injects chat context when `CC_CONNECT_ROOM` is set in the Claude child's environment (set by `cc-connect-tui` when it spawns the child PTY). Unrelated `claude` processes on the box see no chat. |
