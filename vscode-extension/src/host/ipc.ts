// chat.sock client — lifted from chat-ui/src/ipc.ts. The chat-daemon's
// IPC server lives at the path stored in the marker file
// `~/.cc-connect/rooms/<topic>/chat.sock` (the actual socket is under
// `/tmp/cc-<uid>-<rand>.sock` to stay within macOS's SUN_LEN). Wire
// format: one JSON request per line, one JSON response per line.
 ...
 *  connection — same pattern cc-connect-mcp uses. */
export async function ipcCall(
  ...
  if (sock === null) {
    return {
      ok: false,
      err: `no chat.sock marker for topic ${topic.slice(0, 12)}…`,
    };
  }
  return new Promise((resolve) => {
    ...
    let buf = '';
    let resolved = false;
    const finish = (resp: IpcResponse): void => {
      if (resolved) return;
      resolved = true;
      try {
        conn.end();
...
// the UI (not from an AI's MCP-driven send). The dispatch routes onto
